...
%        and doctor sign-off checkboxes for rapid review (<30s).
    ...
    title(ax1, '1. Patient Fundus Photograph', 'FontSize', 13, 'FontWeight', 'bold', 'Color', [0.1, 0.1, 0.15]);

    % --- Panel 2: Explainable AI Heatmap & Lesion Anchors ---
    ax2 = subplot(1, 3, 2);
    imshow(overlay_img, 'Parent', ax2);
    title(ax2, '2. Explainable AI Overlay (Grad-CAM + Lesions)', 'FontSize', 13, 'FontWeight', 'bold', 'Color', [0.1, 0.1, 0.15]);
    ...
    text(0.50, 0.93, triage_text, 'FontSize', 14, 'FontWeight', 'bold', ...
        'Color', 'white', 'HorizontalAlignment', 'center', 'Parent', ax3);
    text(0.50, 0.875, sub_text, 'FontSize', 9, ...
        'Color', [0.95, 0.95, 0.95], 'HorizontalAlignment', 'center', 'Parent', ax3);

    % Clinical Diagnosis Section
    text(0.04, 0.77, 'DIAGNOSIS & AI CONFIDENCE', 'FontSize', 11, 'FontWeight', 'bold', ...
        'Color', [0.2, 0.2, 0.3], 'Parent', ax3);
    text(0.04, 0.70, sprintf('ICDR Severity: %s', grade_str), 'FontSize', 11.5, ...
        'FontWeight', 'bold', 'Color', [0.05, 0.05, 0.1], 'Parent', ax3);
    text(0.04, 0.64, sprintf('AI Confidence: %.1f%% (Empirically Calibrated)', conf_pct), ...
        'FontSize', 10.5, 'Color', [0.2, 0.25, 0.35], 'Parent', ax3);

    % Lesion Evidence Table Section
    rectangle('Position', [0.02, 0.28, 0.96, 0.30], 'Curvature', 0.08, ...
        'FaceColor', [1.0, 1.0, 1.0], 'EdgeColor', [0.85, 0.85, 0.88], 'Parent', ax3);
    text(0.06, 0.54, 'OBJECTIVE LESION EVIDENCE TABLE', 'FontSize', 10, 'FontWeight', 'bold', ...
        'Color', [0.3, 0.35, 0.45], 'Parent', ax3);
    text(0.06, 0.48, sprintf('• Microaneurysms Detected:     %d', n_ma), 'FontSize', 10, 'Parent', ax3);
    text(0.06, 0.43, sprintf('• Hard Exudates (Lipids):        %d', n_hard), 'FontSize', 10, 'Parent', ax3);
    text(0.06, 0.38, sprintf('• Soft Exudates (Cotton-Wool):   %d', n_soft), 'FontSize', 10, 'Parent', ax3);
    text(0.06, 0.33, sprintf('• Retinal Hemorrhages:           %d', n_hem), 'FontSize', 10, 'Parent', ax3);
    text(0.55, 0.48, sprintf('Total Lesion Area: %.2f%%', area_pct), ...
        'FontSize', 10, 'FontWeight', 'bold', 'Color', [0.15, 0.2, 0.4], 'Parent', ax3);
